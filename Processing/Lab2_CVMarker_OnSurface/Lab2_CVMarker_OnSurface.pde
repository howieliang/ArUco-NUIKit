//*********************************************
// Example Code: Lab2_CVMarker_OnSurface
// ArUCo Fiducial Marker Detection in OpenCV Python and then send to Processing via OSC
// Tracking Tangibles on a Surface or Flat Panel Display
...
////set the bundle IDs
int[][] bundlesIDs = {{20}}; 
//int[][] bundlesIDs = {{20},{15},{16},{10}}; //four objects

////set the bundle offsets (unit: m)
PVector[][] bundlesOffsets = {{new PVector(0, 0, -0.126)}}; //(x_offset, y_offset, z_offset)
//PVector[][] bundlesOffsets = {{new PVector(0,0,-0.126)},{new PVector(0,0,-0.046)},{new PVector(0,0,-0.087)},{new PVector(0,0,-0.104)}};
...
//float paperWidthOnScreen = 227.; //if the a4 paper's on-screen width is 227mm

////set the marker width on screen (initial value: 297; unit mm)
float markerWidth = 50; //change this if the marker is of different width

////set the touch threshold (unit: m)
float touchThreshold = 0.015; //change this to adjust sensitivity of touch sensing.

////set the stylus' ID (unit: m)
final int stylus_ID = 20; //set stylus_ID = -1 when there's no stylus
...
void initDataObjects() { //set up the data objects
  ...
  DOlist.add(new DataObject(0, false, 10, width/2-200, height/2-200, 300, "Object 1"));
  DOlist.add(new DataObject(1, false, 10, width/2+200, height/2-200, 300, "Object 2"));
  DOlist.add(new DataObject(2, false, 10, width/2-200, height/2+200, 300, "Object 3"));
  //DataObject(int did, boolean multi, float val, float x, float y, float rz, float w, float h, String name) {
  DOlist.add(new DataObject(3, false, 10, width/2+250, height/2+200, 0, 400, 300, "Object 4"));
}
...
void setup() {
  ...
  calibImg = loadImage("ArUco_Grid50.png"); //select the calibration image
